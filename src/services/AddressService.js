...
export class AddressService extends ApiClientService {
    ...
    loadAliases = address => {
        return this.getApi()
            ...
            .then(aliasesResponse => {
                const lines = aliasesResponse.data.map(
                    item => Alias.fromString(item).alias
                );
                const grouped = groupBy(lines, item =>
    ...
    loadAssets = address => {
        return this.getApi()
            .addresses.assetsBalance(address)
            .then(balanceResponse => {
                const assets = balanceResponse.data.balances.map(item => {
                    const currency = Currency.fromIssueTransaction(
                        item.issueTransaction
                    );
